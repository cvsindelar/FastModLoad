...
EOF
    ...
    if [[ -z ${first_load_arg} && ${autobuild} -eq 0 ]] ; then
        __lmod_execute "${@:1}"
        return
    fi
    ######################
...
To create a fast module for this environment, do "module reset" followed by:
     fml ${ordered_module_list[@]} ${load_arguments[@]}
EOF
            fi
        else
...
function module () {
    ...
    #   which is embedded after this 'if' clause
